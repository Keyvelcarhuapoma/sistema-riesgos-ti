    ...
    <div className="w-64 bg-white border-r border-slate-200 flex flex-col h-full">
      <div className="p-6">
        ...
        <nav className="space-y-1">
          <SidebarItem icon={<Activity size={18} />} text="Panel General" active={true} />
          <SidebarItem icon={<Server size={18} />} text="Activos Críticos" active={false} />
          <SidebarItem icon={<ShieldAlert size={18} />} text="Vulnerabilidades" active={false} />
          <SidebarItem icon={<FileText size={18} />} text="Auditoría & Logs" active={false} />
        </nav>
      </div>
...
function SidebarItem({ icon, text, active }: { icon: React.ReactNode, text: string, active: boolean }) {
  return (
    <a href="#" className={`flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${active ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
